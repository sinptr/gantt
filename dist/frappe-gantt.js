        ...
        value: function update_bar_position(_ref) {
            ...
            if (x) {
                ...
                var xs = Array.from(this.task.dependencies, function (_ref2) {
                    ...
                    return _this3.gantt.get_bar(dep).x;
                });
    ...
    createClass(Gantt, [{
            ...
            try {
                var _loop = function _loop() {
                    ...
                    var arrows = Array.from(task.dependencies, function (_ref) {
                        ...
                        type);
                        _this3.layers.arrows.appendChild(arrow.element);
                        return arrow;
        ...
        value: function bind_bar_events() {
            ...
            $.on(this.$svg, 'mousedown', '.handle-group .circle', function (e, element) {
                ...
                var bar = _this6.get_bar(task_id);
                if (is_connecting) {
                    connecting_bar = bar;
                    connecting_type = element.classList.contains('left') ? types.START_TO_START : types.END_TO_START;
                    element.classList.add('selected');
                    _this6.$svg.querySelectorAll('.bar-wrapper').forEach(function (wrapper) {
                        var id = wrapper.getAttribute('data-id');
                        var can_add_dependency = _this6.can_add_dependency(connecting_bar.task, _this6.get_task(id));
                    ...
                    _this6.$svg.querySelectorAll('.handle-group .circle').forEach(function (circle) {
                        circle.classList.remove('disabled', 'active', 'selected');
                    });

                    if (_this6.can_add_dependency(connecting_task, _task)) {
                        _this6.add_dependency(connecting_task, _task, connecting_type);
                    }
                    connecting_bar = null;
                }
            });
            ...
            $.on(this.$svg, 'mousemove', function (e) {
                ...
                if (_this6.options.is_draggable && is_dragging) {
                    var row = Math.floor((e.offsetY - _this6.options.header_height - 10) / (_this6.options.bar_height + _this6.options.padding));
                    if (row >= 0 && row < _this6.tasks.length) {
                        var _bar = _this6.get_bar(parent_bar_id);
                        var offset = row - _bar.task._index;

                        if (offset) {
                            new_position = _bar.task._index + offset;
                            _this6.tasks.splice(new_position, 0, _this6.tasks.splice(_bar.task._index, 1)[0]);

                            _this6.tasks.forEach(function (task, i) {
                                if (task._index !== i) {
                                    task._index = i;
                                    var _bar2 = _this6.get_bar(task.id);
            ...
            $.on(this.$container, 'scroll', function (e) {
                ...
                requestAnimationFrame(function () {
                    _this6.layers.date.setAttribute('transform', 'translate(0,' + scrollTop + ')');
                });
            });
            ...
            $.on(this.$svg, 'mouseup', function (e) {
                ...
                if (new_position !== null) {
                    _this6.trigger_event('order_change', [parent_bar_id, new_position]);
                    new_position = null;
                }
                bars.forEach(function (bar) {
                    var $bar = bar.$bar;
