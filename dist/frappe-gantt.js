(function (global, factory) {
    ...
    _createClass(Bar, [{
      ...
      value: function update_bar_position(_ref) {
        ...
        if (x) {
          ...
          this.x = Math.max.apply(Math, _toConsumableArray(xs).concat([x]));
      ...
      key: "update_deps",
      value: function update_deps() {
        var _this4 = this;

        var excludedTaskIds = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : new Set();
        var _this$task$dependenci = this.task.dependencies,
            dependencies = _this$task$dependenci === void 0 ? [] : _this$task$dependenci;

        var newDeps = _toConsumableArray(dependencies).map(function (_ref5) {
          ...
          var parent = _this4.gantt.get_task(id);

          var type = info.type;

          var offset = _this4.gantt.getTaskOffset(parent, _this4.task, type);

          return [id, _objectSpread2({}, info, {
            offset: offset
          })];
        });
      ...
      value: function date_changed(excludedTaskIds) {
        ...
        if (isSideEffect) {
          ...
          if (nonEmptyOffsets.length) {
            ...
            var parentEnd = parent._end;
            new_start_date = this.gantt.calendar.addWithWeekendsSkip(this.gantt.getStartDateForOffset(parentStart, parentEnd, type), offset);

            if (new_start_date < parentStart) {
              this.task.dependencies.set(parentId, {
                type: type,
      ...
      value: function compute_start_end_date() {
        ...
        var new_start_date = moment(this.gantt.gantt_start).add(x_in_units * this.gantt.options.step, 'hours').toDate();
        var width_in_units = bar.getWidth() / this.gantt.options.column_width;
        var new_end_date = moment(new_start_date).add(width_in_units * this.gantt.options.step, 'hours').toDate();
        return {
    ...
    _createClass(Arrow, [{
      ...
      value: function calculate_path() {
        ...
        if (start_x + start_padding >= end_x) {
          ...
          offset *= sign;
          this.path = "\n                M ".concat(start_x, " ").concat(start_y, "\n                h ").concat(start_padding, "\n                V ").concat(end_y - offset, "\n                H ").concat(end_x - start_padding * typeSign, "\n                V ").concat(end_y, "\n                L ").concat(end_x, " ").concat(end_y, "\n                ").concat(arrowPath);
        } else {
          this.path = "\n                M ".concat(start_x, " ").concat(start_y, "\n                h ").concat(start_padding, "\n                V ").concat(end_y, "\n                L ").concat(end_x, " ").concat(end_y, "\n                ").concat(arrowPath);
        }
      }
  ...
  function () {
    ...
    function Calendar(holidays, format, workingHours) {
      ...
      var _workingHours = _slicedToArray(workingHours, 2);
    ...
    _createClass(Calendar, [{
      ...
      key: "addWithWeekendsSkip",
      value: function addWithWeekendsSkip(start, hours) {
        var s = moment(start);
        var sign = Math.sign(hours);
        var workingHours = 24;
        var hoursToAdd = Math.abs(hours); // skip holidays if start date is holiday
    ...
    _createClass(Gantt, [{
      ...
      value: function setup_tasks(tasks) {
        ...
        this.tasks = tasks.map(function (task, i) {
          ...
          task.duration = _this.calendar.computeTaskDuration(task._start, task._end); // make task invalid if duration too large
          ...
          try {
            if (Array.isArray(task.dependencies)) {
              task.dependencies = task.dependencies.map(function (_ref) {
      ...
      value: function setup_dependencies() {
        ...
        this.tasks.forEach(function (_ref3) {
          ...
          dependencies.forEach(function (info, taskId) {
            var map = dependencyMap.get(taskId) || new Map();
            var offset = info.offset;

            if (!offset && offset !== 0) {
              offset = _this2.getTaskOffset(_this2.get_task(taskId), _this2.get_task(id), info.type);
              dependencies.set(taskId, _objectSpread2({}, info, {
                offset: offset
              }));
            }

            map.set(id, _objectSpread2({}, info, {
              offset: offset
            }));
      ...
      value: function setup_calendar() {
        var _this$options = this.options,
            calendar = _this$options.calendar,
            workStartHour = _this$options.workStartHour,
            workEndHour = _this$options.workEndHour,
            date_format = _this$options.date_format;
      ...
      value: function bind_scroll() {
        ...
        $.on(this.$container, 'scroll', function (e) {
          ...
          _this3.layers.date.setAttribute('transform', "translate(0,".concat(scrollTop, ")"));
        });
      ...
      value: function bind_resize() {
        ...
        this.$svg.addEventListener('wheel', function (e) {
          ...
          if (e.ctrlKey && _this4.options.view_mode === 'Day' && false) {
            e.preventDefault();
            _this4.options.column_width -= e.deltaY * 0.05;

            if (_this4.options.column_width < 16) {
              _this4.options.column_width = 16;
            }
            ...
            frame = requestAnimationFrame(function () {
              _this4.bars.forEach(function (bar) {
              ...
              _this4.layers.date.querySelectorAll('.lower-text').forEach(function (date, i) {
                date.setAttribute('x', i * _this4.options.column_width + _this4.options.column_width / 2);
              });
            });
          }
        });
      ...
      value: function setup_layers() {
        this.layers = {};
        var layers = ['grid', 'arrows', 'progress', 'bar', 'details', 'date']; // make group layers
      ...
      value: function make_grid_ticks() {
        ...
        try {
          for (var _iterator2 = this.dates[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      value: function make_dates() {
        ...
        try {
          for (var _iterator3 = this.get_dates_to_draw()[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      ...
      value: function get_dates_to_draw() {
        ...
        return this.dates.map(function (date, i) {
          var d = _this5.get_date_info(date, last_date, i);
      ...
      value: function make_arrows() {
        ...
        try {
          var _loop = function _loop() {
            ...
            var arrows = Array.from(task.dependencies, function (_ref4) {
              var _ref5 = _slicedToArray(_ref4, 2),
                  task_id = _ref5[0],
                  type = _ref5[1].type;

              var dependency = _this7.get_task(task_id);

              if (!dependency) return;
              var arrow = new Arrow(_this7, _this7.bars[dependency._index], // from_task
              _this7.bars[task._index], // to_task
              type);

              _this7.layers.arrows.appendChild(arrow.element);

              return arrow;
            }).filter(Boolean); // filter falsy values

            _this7.arrows = _this7.arrows.concat(arrows);
          };
      ...
      value: function make_arrow(from_task, to_task, type) {
        ...
        this.arrows = [].concat(_toConsumableArray(this.arrows), [arrow]);
      }
    }, {
      ...
      value: function map_arrows_on_bars() {
        ...
        try {
          var _loop2 = function _loop2() {
            ...
            bar.arrows = _this8.arrows.filter(function (arrow) {
              return arrow.from_task.task.id === bar.task.id || arrow.to_task.task.id === bar.task.id;
            });
          };

          for (var _iterator5 = this.bars[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
            _loop2();
          }
        } catch (err) {
      ...
      value: function set_scroll_position() {
        ...
        setTimeout(function () {
          return parent_element.scrollTo({
            left: scroll_pos
          });
        });
      }
      ...
      value: function bind_bar_events() {
        ...
        $.on(this.$svg, 'mousedown', '.handle-group .circle', function (e, element) {
          ...
          var bar = _this10.get_bar(task_id);
          ...
          if (is_connecting) {
            ...
            _this10.$svg.querySelectorAll('.bar-wrapper').forEach(function (wrapper) {
              var id = wrapper.getAttribute('data-id');

              var can_add_dependency = _this10.can_add_dependency(connecting_bar.task, _this10.get_task(id));
            ...
            _this10.$svg.querySelectorAll('.handle-group .circle').forEach(function (circle) {
              circle.classList.remove('disabled', 'active', 'selected');
            });

            if (_this10.can_add_dependency(connecting_task, task)) {
              _this10.add_dependency(connecting_task, task, connecting_type);
            }
        ...
        $.on(this.$svg, 'mousedown', '.bar-wrapper, .handle', function (e, element) {
          ...
          var _getOffset2 = _slicedToArray(_getOffset, 2);
          ...
          if (_this10.dependency_map.has(parent_bar_id)) {
            if (is_resizing_left || is_resizing_right) {
              _this10.dependency_map.get(parent_bar_id).forEach(function (_ref6, childTaskId) {
                var type = _ref6.type;

                if (type === allowType) {
                  ids = [].concat(_toConsumableArray(ids), [childTaskId], _toConsumableArray(_this10.get_all_dependent_tasks(childTaskId)));
                }
              });
            } else {
              ids = [].concat(_toConsumableArray(ids), _toConsumableArray(_this10.get_all_dependent_tasks(parent_bar_id)));
            }
          } ///


          ids = _toConsumableArray(new Set(ids).values());
          bars = ids.map(function (id) {
            return _this10.get_bar(id);
          });
          _this10.bar_being_dragged = parent_bar_id;
        ...
        $.on(this.$svg, 'mousemove', function (e) {
          ...
          var _getOffset3 = getOffset(e),
              _getOffset4 = _slicedToArray(_getOffset3, 2),
              offsetX = _getOffset4[0],
              offsetY = _getOffset4[1];

          var dx = offsetX - x_on_start;

          if (_this10.options.is_draggable && _this10.options.is_sortable && is_dragging) {
            var row = Math.floor((e.offsetY - _this10.options.header_height - 10) / (_this10.options.bar_height + _this10.options.padding));

            if (row >= 0 && row < _this10.tasks.length) {
              var bar = _this10.get_bar(parent_bar_id);

              var offset = row - bar.task._index;

              if (offset) {
                new_position = bar.task._index + offset;

                _this10.tasks.splice(new_position, 0, _this10.tasks.splice(bar.task._index, 1)[0]);

                _this10.tasks.forEach(function (task, i) {
                  if (task._index !== i) {
                    task._index = i;

                    var _bar = _this10.get_bar(task.id);

                    _bar.update_bar_position({
                      y: _bar.compute_y()
                    });
                  }
                });
          ...
          bars.forEach(function (bar) {
            var $bar = bar.$bar;
            $bar.finaldx = _this10.get_snap_position(dx);
            ...
            } else if (is_dragging && _this10.options.is_draggable) {
              bar.update_bar_position({
                x: $bar.ox + $bar.finaldx
              });
            }
        ...
        $.on(this.$svg, 'mouseup', function (e) {
          if (_this10.bar_being_dragged) {
            _this10.bar_being_dragged = null;

            if (new_position !== null) {
              _this10.trigger_event('order_change', [parent_bar_id, new_position]);
      ...
      value: function bind_bar_progress() {
        ...
        $.on(this.$svg, 'mousedown', '.handle.progress', function (e, handle) {
          ...
          var _getOffset6 = _slicedToArray(_getOffset5, 2);
        ...
        $.on(this.$svg, 'mousemove', function (e) {
          ...
          var _getOffset7 = getOffset(e),
              _getOffset8 = _slicedToArray(_getOffset7, 2),
      ...
      value: function get_all_dependent_tasks(task_id) {
        ...
        while (tasksToProcess.size > 0) {
          _toConsumableArray(tasksToProcess.values()).forEach(function (id) {
      ...
      value: function view_is(modes) {
        ...
        if (Array.isArray(modes)) {
          return modes.some(function (mode) {
            return _this12.options.view_mode === mode;
          });
        }
      ...
      value: function add_dependency(task_from, task_to, type) {
        ...
        bar.date_changed();
        var offset = this.getTaskOffset(task_from, task_to, type);
        task_to.dependencies.set(task_from.id, {
          type: type,
          offset: offset
        });
        this.setup_dependencies();
        var dependentTasks = this.get_all_dependent_tasks(task_to.id);
        dependentTasks.add(task_to.id);
        dependentTasks.forEach(function (id) {
          var depBar = _this13.get_bar(id);

          depBar.update_bar_position({
            x: depBar.compute_x()
          });
      ...
      value: function delete_dependency(task_from, task_to, info) {
      ...
      key: "getStartDateForOffset",
      value: function getStartDateForOffset(start, end, type) {
        return type === enums.dependency.types.START_TO_START ? start : end;
      }
    }, {
      key: "getTaskOffset",
      value: function getTaskOffset(taskFrom, taskTo, type) {
        var offset = this.calendar.getBusinessHoursRange(this.getStartDateForOffset(taskFrom._start, taskFrom._end, type), taskTo._start);
        return offset;
      }
    }, {
      key: "getTasks",
